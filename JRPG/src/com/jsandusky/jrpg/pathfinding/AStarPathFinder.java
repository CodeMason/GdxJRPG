...
import com.jsandusky.util.Profiler;
...
public class AStarPathFinder implements PathFinder {
	...
	private ArrayList<Node> closed = new ArrayList<Node>(512);
	/** The set of nodes that we do not yet consider fully searched */
	private SortedList open = new SortedList();
	
	/** The map being searched */
	private PathTileMap map;
	 ...
	 */
	public AStarPathFinder(PathTileMap map, int maxSearchDistance, boolean allowDiagMovement) {
	 ...
	 * @param allowDiagMovement True if the search should try diaganol movement
	 */
	public AStarPathFinder(PathTileMap map, int maxSearchDistance, 
	...
	public Path findPath(Object mover, int sx, int sy, int tx, int ty) {
		...
		if (map.blocked(mover, tx, ty)) {
			Profiler.inst().bump("AStar_Fails");
			return null;
		}
		Profiler.inst().start("AStar_FindPath");
		...
		while ((maxDepth < maxSearchDistance) && (open.size() != 0)) {
			...
			Profiler.inst().start("AStar_CoreLoop");
			...
			Profiler.inst().stop("AStar_CoreLoop");
		}

		// since we've got an empty open list or we've run out of search 
		// there was no path. Just return null
		if (nodes[tx][ty].parent == null) {
			Profiler.inst().stop("AStar_FindPath");
			Profiler.inst().bump("AStar_Fails");
			return null;
		}
		...
		Profiler.inst().start("AStar_BuildPath");
		Path path = new Path();
		...
		Profiler.inst().stop("AStar_BuildPath");
