...
public class Heap {
	...
	public synchronized void buildHeap () {
		for ( int i = (int)Math.floor ( count_ / 2. ); i >= 0; i-- ) {
			heapify ( i );
		}
	}
	...
	public synchronized void heapify ( int k ) {
		int l = left ( k );
		int r = right ( k );
		int largest = k;
		if ( l < count_ && compare ( nodes_[l], nodes_[k]) > 0 ) {
			largest = l;
		} else {
			largest = k;
		}
		if ( r < count_ && compare ( nodes_[r], nodes_[largest] ) > 0 ) {
			largest = r;
		}
		if ( largest != k ) {
			Object tmp = nodes_[k];
			nodes_[k] = nodes_[largest];
			nodes_[largest] = tmp;
			heapify ( largest );
		}
	}
	...
	/*
	 ...
	 7.     then largest = r
	 8. if largest  == i
	 9.     then exchange A[i] <=>ª A[largest]
