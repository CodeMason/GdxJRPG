package com.jsandusky.fscript;

import java.util.ArrayList;
import java.lang.reflect.Method;

import com.jsandusky.fscript.introspection.IntrospectorBase;
 ...
 */
 ...
 added import("shortName","fullpackagename")
 */
public class FSReflectionExtension implements FSParserExtension {
    ...
    {
        public void handle( String name, Exception exc )
            throws FSException
        {
            throw new FSException( "Error calling method " + name + " " + exc.getMessage(), exc );
        }
    };
        ...
        if (pos>0) {
            ...
            if (object != null && object instanceof FSObject) {
				...
				while (name.indexOf('.') > 0) {
					String partName = name.substring(0,name.indexOf('.'));
					name = name.substring(pos+1);
					object = getObjectVar(object,partName);
				}
        ...
        if (pos > 0) {
            ...
            if (object!=null && object instanceof FSObject) {
				...
				while (name.indexOf('.') > 0) {
					String partName = name.substring(0,name.indexOf('.'));
					name = name.substring(pos+1);
					object = getObjectVar(object,name);
				}				
    ...
    public Object getVar(String name, Object index) throws FSException {
		...
		if (val.getClass().isArray()) {
			return Array.get(val,(Integer)index);
		}
        return null;
    }
    ...
    public void setVar(String name, Object index, Object value) throws FSException {
        Object val = getVar(name);
		if (val.getClass().isArray()) {
			Array.set(val,(Integer)index,value);
		}
    }
    ...
    public Object callFunction(String name, ArrayList params) throws FSException {
        ...
        if (pos > 0) {
			...
			String nm = (String)params.get(1);
			if (imports.containsKey(val))
				throw new FSException("Duplicate import for type: " + val);
			imports.put(val,nm);
		}
        throw new FSUnsupportedException();
    }
    ...
    throws FSException {
        try {
            ...
            throw new FSException("Error Creating new object "+e.getMessage(),e);
        }
    ...
    private Class getClass(String name) throws FSException {
        try {
			if (!name.contains(".") && imports.containsKey(name))
				name = imports.get(name);
            return Thread.currentThread().getContextClassLoader().loadClass(name);
        } catch (Exception e) {
            throw new FSException("Error getting class "+name+" "+e.getMessage(),e);
        }
    ...
    private boolean checkMethods(Class[] c, Object[] o) {
        ...
        for (n=0 ; n<len ; n++) {
            ...
            !(c[n].equals(Float.TYPE)&&o[n] instanceof Float)) {
                return false;
            }
        }
    ...
    /*
     * Ensures the right type is passed back i.e. Integer,String,Float
     * or FSObject
     */
    private Object normalizeObj(Object o, Class c) {
        // Return the right type...
        if (o instanceof Integer || o instanceof String || o instanceof Float) {
            return o;
        } else {
