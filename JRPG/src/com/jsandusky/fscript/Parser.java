 ...
 * 04.12.2001
 *   - allow add/subtract/multiply/divide int with Float
 * 24.04.2002
...
final class Parser {
                    ...
                    if (!prevOp){
                        ...
                        if (negate) {
                            if (val instanceof Integer){
                                ...
                                val=new Float(-((Float)val).floatValue());
                            } else {
    ...
    private Object evalPlus(Object lVal,Object rVal) throws FSException{
        if (lVal instanceof Integer && rVal instanceof Integer){
            ...
            return new Float(((Float)lVal).floatValue()
            +((Float)rVal).floatValue());
        } else if (lVal instanceof String || rVal instanceof String){
            return new String(lVal.toString()+rVal.toString());
        } else if (lVal instanceof Float && rVal instanceof Integer){
            return new Float(((Float)lVal).floatValue()
            +((Integer)rVal).intValue());
        } else if (lVal instanceof Integer && rVal instanceof Float){
            return new Float(((Integer)lVal).intValue()
            +((Float)rVal).floatValue());
        } else {
    ...
    private Object evalLs(Object lVal,Object rVal) throws FSException{
        if (lVal instanceof Integer && rVal instanceof Integer){
            ...
            if (((Float)lVal).floatValue()<((Float)rVal).floatValue()){
                return FS_TRUE;
            } else {
                return FS_FALSE;
            }
...
/*    private void printETree(ETreeNode node){
    ...
    // OR you can assign a String, Integer or Float to an FSObject if the contained object already
